...
def audio_callback(in_data, frame_count, time_info, status):
    ...
    while note_time_dt < audio_time + frame_count:
        ...
        for note in notes:
            freq =  int(2 * 38.89 * pow(2.0, note / 12.0) / sample_rate)
    ...
    for t,f in  audio_notes:
        ...
        if instrument == 0:
            w = np.sign(1 - np.mod(x * f, 2))
        elif instrument == 1:
            w = np.mod(x * f - 1, 2) - 1
        elif instrument == 2:
            w = 2*np.abs(np.mod(x * f - 0.5, 2) - 1) - 1 
        elif instrument == 3:
            w = np.sin(x * f * math.pi) 
...
def draw_sliders():
	for i in range(slider_num):
		...
		pygame.draw.circle(screen, slider_color, (int(cx), py), int((slider_w - tick_pad)/2))
